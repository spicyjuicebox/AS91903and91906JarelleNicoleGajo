...
# Ordering
# Route for Ordering
@views.route("/order", methods=['GET', 'POST']) # The GET and POST methods are used.
@login_required
def menu(): # The 'menu' function defined.
    if request.method == 'POST':
        order = request.form.get('order') # The order of the user is stored in the order form.
        total = request.form.get('total') # The total price of the order is stored in the total form.
        if order and total:
            return redirect(url_for('views.payment_option', order=order, total=total)) # After the user has selected their order and the total is successfully calculated, they will be directed to the payment option page. The order and total will be brought to the payment option page.
    return render_template("order.html", user=current_user) # Directing the user to the Order page.

# Route for Choosing the Payment Option
@views.route('/payment_option', methods=['GET', 'POST'])
@login_required
def payment_option(): # The 'payment_option' function defined.
    order = request.args.get('order') # Bringing the order from the order page.
    total = request.args.get('total') # Bringing the total price from the order page.
    if not order or not total:
        flash('No order found.', category='error')
        return redirect(url_for('views.menu'))
    return render_template('payment_option.html', user=current_user, order=order, total=total) # Directing the user to the payment option page.

# Route for Cash Payment
@views.route('/cash_payment', methods=['GET', 'POST'])
@login_required
def cash_payment(): # The 'cash_payment' function defined.
    order = request.args.get('order') # Bringing the order from the order page.
    total = request.args.get('total') # Bringing the total price from the order page.
    if not order or not total:
        flash('No order found.', category='error')
        return redirect(url_for('views.menu'))
    #new_order = Order(items=order, user_id=current_user.id, user_email=current_user.email, payment_method='Cash', total_price=total) # New order storing the items, user_id, user_email, cash payment method and total price.
    #db.session.add(new_order) # The order that is stored in the 'new_order' will be added to the database.
    #db.session.commit() # After the cash payment is processed, the order will be added to the database.
    return render_template('cash_payment.html', user=current_user, order=order, total=total, total_price=total) # Directing the user to the cash payment page. The total_price brings the total price from the cart.

# Route for Processing the Cash Payment
@views.route('/process_cash_payment', methods=['POST'])
@login_required
def process_cash_payment(): # The 'process_cash_payment' function defined.
    order = request.form.get('order') # The order is stored in the order form.
    total = request.form.get('total') # The total price is stored in the total form.
    new_order = Order(items=order, user_id=current_user.id, user_email=current_user.email, payment_method='Card', total_price=total)
    db.session.add(new_order) # The order that is stored in the 'new_order' will be added to the database.
    db.session.commit() # When the cash payment is processed, the order is added to the database.
    return redirect(url_for('views.thank_you'))

# Route for Card Payment
@views.route('/card_payment', methods=['GET', 'POST'])
@login_required
def card_payment(): # The 'card_payment' function defined.
    order = request.args.get('order') # Bringing the order from the order page.
    total = request.args.get('total') # Bringing the total price from the order page.
    if not order or not total:
        flash('No order found.', category='error')
        return redirect(url_for('views.menu'))
    #new_order = Order(items=order, user_id=current_user.id, user_email=current_user.email, payment_method='Card', total_price=total) # New order storing the items, user_id, user_email, card payment method and total price.
    return render_template('card_payment.html', user=current_user, order=order, total=total, total_price=total) # Directing the user to the card payment page. The total_price brings the total price from the cart.

# Route for Processing the Card Payment
@views.route('/process_card_payment', methods=['POST'])
@login_required
def process_card_payment(): # The 'process_card_payment' function defined.
    order = request.form.get('order') # The order is stored in the order form.
    total = request.form.get('total') # The total price is stored in the total form.
    new_order = Order(items=order, user_id=current_user.id, user_email=current_user.email, payment_method='Card', total_price=total)
    db.session.add(new_order) # The order that is stored in the 'new_order' will be added to the database.
    db.session.commit() # When the card payment is processed, the order is added to the database.
    return redirect(url_for('views.thank_you'))

# Route for the Thank You Page
@views.route('/thankyou', methods=['GET'])
@login_required
def thank_you(): # The 'thank_you' function defined.
    return render_template('thankyou.html', user=current_user) # After paying, the user will be directed to the thank you page.
# Ordering End
